        ...
        prompt = [
            {
                ...
                "content": (
                    ...
                    "任务是基于当前当事人的本轮对话，只提取本轮明确新增、修改或取消的诉求事项："
                    "1. currentDemand 是当前当事人已有诉求；本轮没有提到的事项不要输出，保持原样；"
                    ...
                    "8. 如果本轮明确表示不再要求某事项，该事项 content 返回无；"
                    "9. 如果 currentDemand 为空，首次提取需要输出五个固定事项，未提及的 content 返回无；"
                    "10. 输出字段只需要 claims 和 changed_topics。claims 必须是数组，每项包含 topic 和 content；"
                    "changed_topics 必须只包含本轮新增、修改或取消的事项。"
                ),
            },
            {
                "role": "user",
        ...
        prompt = [
            {
                ...
                "content": (
                    ...
                    "只判断输入 rows 中双方诉求语义是否一致，不要补写或改写任一方内容。"
                    "topic 只能是：道歉、赔偿金额、履行方式、后续承诺、其他。"
                    "一致不是文字完全相同，而是双方对该事项的意思、金额、履行方式或承诺已经达成一致。"
                    "双方都是“无”视为一致；一方有具体要求、另一方拒绝或金额/方式/期限不同，视为待协商。"
                    "输出字段：commonGrounds, disputePoints, feasibility。"
                    "commonGrounds/disputePoints 必须是数组，每项包含 topic，topic 必须取固定事项；不要输出 rows 以外的事项。"
                ),
            },
            {
                "role": "user",
